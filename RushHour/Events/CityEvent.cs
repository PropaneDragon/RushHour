    ...
    internal abstract class CityEvent
    {
        ...
        public bool Register(uint citizenID, ref Citizen person)
        {
            ...
            if (m_eventData.m_registeredCitizens < GetCapacity())
            {
                ...
                if(registered)
                {
                    ...
                    LoggingWrapper.Log("Registered citizen to event (" + m_eventData.m_registeredCitizens + "/" + GetCapacity() + ")");
                }
            }
        ...
        public void Update()
        {
            if (CityEventManager.CITY_TIME > m_eventData.m_eventStartTime && !m_eventData.m_eventStarted && !m_eventData.m_eventEnded)
            {
                ...
                LoggingWrapper.Log("Event starting at " + eventBuilding.Info.name);
            ...
            else if (m_eventData.m_eventStarted && CityEventManager.CITY_TIME > m_eventData.m_eventFinishTime)
            {
                ...
                if ((eventBuilding.m_flags & Building.Flags.Created) != Building.Flags.None)
                {
                    ...
                    LoggingWrapper.Log("Event finished at " + eventBuilding.Info.name);
                }
                else
                {
                    LoggingWrapper.Log("Event finished at a building that no longer exists...");
                }
        ...
        public virtual string GetCitizenMessageStarted()
        {
            ...
            if (m_eventStartedMessages.Count > 0)
            {
                ...
                {
                    LoggingWrapper.LogWarning("Event " + m_eventData.m_eventName + " has less than 4 messages for the start. This could get boring.");
                }
            ...
            LoggingWrapper.Log("Event chirped started \"" + chosenMessage + "\"");
            return chosenMessage;
        }
